...
import { appLogFilterSchema } from "@repo/api";
...
export function DataTableFilterCommand<TSchema extends z.AnyZodObject>({
  ...
  // Determine the table type directly from the schema parameter
  const tableType = schema === appLogFilterSchema ? "app" : "request";
  ...
  // Filter out timestamp and disabled fields
  const filterFields = useMemo(
    () => _filterFields?.filter((i) => !i.commandDisabled && i.value !== "timestamp"),
    [_filterFields],
  );
