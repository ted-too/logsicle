...
const getMenuList = (params: { projId: string; pathname: string }) => [
  ...
  {
    title: "Events",
    url: `/dashboard/${params.projId}/events`,
    icon: Brodcast,
    active: params.pathname.includes(`/dashboard/${params.projId}/events`),
  },
  {
    title: "Logs",
    url: `/dashboard/${params.projId}/logs`,
    icon: LogsIcon,
    active: params.pathname.includes(`/dashboard/${params.projId}/logs`),
  },
  {
    title: "Traces",
    url: `/dashboard/${params.projId}/traces`,
    icon: TracesIcon,
    active: params.pathname.includes(`/dashboard/${params.projId}/traces`),
  },
  {
    title: "Alerts",
    url: `/dashboard/${params.projId}/alerts`,
    icon: Warning2,
  ...
  return items.map((item) => (
    <SidebarMenuItem key={item.title} className="w-full">
      <SidebarMenuButton className="rounded-lg" asChild>
        <Link data-active={item.active} to={item.url}>
          <item.icon
...
export function AppSidebar({ user }: { user: User }) {
  ...
  const menuList = getMenuList({ projId, pathname });
        ...
        >
          <SidebarContent className="justify-between">
            <SidebarGroup className="p-0">
              <SidebarGroupContent>
                <SidebarMenuItem className="w-full p-2 pb-0">
                ...
                <SidebarMenu className="items-center gap-3 px-2 pt-3">
                  <MenuItems items={menuList.slice(0, 1)} pathname={pathname} />
                  <SidebarSeparator className="w-full" />
                  <MenuItems
                    items={menuList.slice(1, menuList.length)}
                    pathname={pathname}
                  />
                </SidebarMenu>
              </SidebarGroupContent>
